import {
  AnatomyQueryWithConstraintsSchema,
  type AnatomyDraftInput,
  type AnatomyNode,
  type AnatomyQueryWithConstraints,
...
import { AnatomyQueryPathError } from "./AnatomyQueryPathError.js";
import { evaluateName } from "./evaluateName.js";
...
export const queryAnatomy = (
  ...
  path: string,
  hooks: { skipValidation?: boolean; matchName?: typeof evaluateName } = {},
): Result<
  AnatomyQueryWithConstraints,
  AnatomyValidationIssue[] | AnatomyQueryPathError
> => {
  ...
  const matchName = hooks.matchName ?? evaluateName;
  ...
  const result: AnatomyQueryWithConstraints = {
    ...
    reason: null,
    captures: {},
    bindings: definition.structure.bindings ?? {},
    policies: defaults,
    ancestors: [],
    rules: [],
    matches: [],
  ...
  const policiesFor = (node?: AnatomyNode) => {
    const resolved = resolveAnatomyPolicies(
      defaults,
      policyAncestors,
      node && node.kind !== "one_of"
        ? { id: node.id, overrides: node.policyOverrides }
        : undefined,
    );
    return {
  ...
  const describe = (nodes: AnatomyNode[]) =>
    nodes.map((node) => {
      const expectedName =
        node.kind === "one_of"
          ? null
          : node.name.type === "literal"
            ...
            : node.name.value.replace(
                ...
                (placeholder, name: string) =>
                  result.captures[name] ?? placeholder,
              );
      return {
        rulePath: registry.get(node.id)!.rulePath,
        node,
        policies: policiesFor(node),
        expectedName,
        ...(node.kind === "file" && node.exports
          ? {
              expectedExport: resolveAnatomyExportName(
                node.exports,
                expectedName!,
                result.captures,
              ),
  ...
  for (const [index, part] of parts.entries()) {
    ...
    const candidates = entries.flatMap(({ node, inGroup }) => {
      const exact = matchName(node, part, result.captures, result.bindings);
      const match =
        exact.kind === "none" && !inGroup
          ? matchName(node, part, result.captures, result.bindings, true)
          : exact;
      return match.kind === "none"
        ? []
        : [{ node, match, caseMismatch: exact.kind === "none" }];
    });
    result.matches = candidates.map(
      ({ node }) => registry.get(node.id)!.rulePath,
    );
    if (candidates.length !== 1) {
      result.status =
        candidates.length === 0 ? QueryStatus.unmatched : QueryStatus.ambiguous;
      result.reason =
        candidates.length === 0
    ...
    for (const group of nodes) {
      if (
        group.kind === "one_of" &&
        group.alternatives.some((entry) => entry.id === node.id)
      ) {
        result.groups.push({
          rulePath: registry.get(group.id)!.rulePath,
          minimumMatches: group.minimumMatches,
          maximumMatches: group.maximumMatches,
          alternatives: group.alternatives.map(
            (entry) => registry.get(entry.id)!.rulePath,
          ),
        });
      }
    }
