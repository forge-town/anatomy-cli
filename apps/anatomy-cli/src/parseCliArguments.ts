...
import { AnatomyCliArgumentError } from "./AnatomyCliArgumentError";
import { AnatomyCliFormatValues } from "./AnatomyCliFormatValues";
...
export const parseCliArguments = (
  ...
  if (options.gitFiles && options.ignore.length > 0) {
    return err(new AnatomyCliArgumentError("--git-files cannot be combined with --ignore"));
  }
  return ok(options);
};
