...
const StartupGridBackground = ({ gridRef }: { gridRef: GridRef }) => (
  <div
    ref={gridRef}
    aria-hidden="true"
...
const StructureScanPreview = ({ onStageChange }: { onStageChange?: (stage: number) => void }) => {
  ...
  return (
    <div
      ...
      className="grid overflow-hidden bg-[var(--line-surface)] text-left md:grid-cols-2"
    >
      <div className="border-b border-[var(--line-border)] px-5 py-6 md:border-b-0 md:border-r md:px-8 md:py-8">
        <div className="flex h-8 items-center justify-between gap-4 border-b border-[var(--line-border)]">
          ...
          <Button
            aria-label={
              showJson ? t("startup.definitionHumanToggle") : t("startup.definitionJsonToggle")
            }
        ...
        <div className="relative mt-5 h-[20rem] overflow-hidden">
          <div
            ...
            className={`absolute inset-0 overflow-y-auto pr-3 transition-[opacity,transform] duration-300 ease-out ${showJson ? "pointer-events-none -translate-y-2 opacity-0" : "translate-y-0 opacity-100"}`}
          >
            <p className="text-base font-medium leading-7 tracking-[-0.01em] text-[var(--line-foreground)]">
              {t("startup.definitionHumanIntro")}
            </p>
            <div className="mt-5 space-y-3 font-mono text-xs text-[var(--line-muted)]">
              {humanDefinitionRows.map((row, index) => (
                <div
                  className="flex items-center gap-3 border-b border-[var(--line-border)] pb-3 last:border-b-0"
                  key={row.key}
                >
                  <span className="w-5 shrink-0 text-[10px] text-[var(--line-accent)]">
                    0{index + 1}
                  </span>
                  ...
                  <CheckCircle2
                    aria-hidden="true"
                    className="ml-auto size-3.5 shrink-0 text-[var(--line-success)]"
                  />
                </div>
              ))}
            </div>
          ...
          <pre
            aria-hidden={!showJson}
            className={`absolute inset-0 overflow-y-auto overflow-x-hidden overscroll-contain whitespace-pre-wrap break-words pr-3 font-mono text-[11px] leading-5 transition-[opacity,transform] duration-300 ease-out sm:text-xs ${showJson ? "translate-y-0 opacity-100" : "pointer-events-none translate-y-2 opacity-0"}`}
          >
            {definitionExample}
          </pre>
        </div>
          ...
          {scanRows.map((row, index) => (
            <div
              className={cn(
                ...
                scanStage > index ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0",
              )}
              key={row.label}
            >
              <StructureTreeRow
...
const StartupScanSection = () => {
  ...
  return (
    <section
      id="scan"
      className="relative w-full border-y border-[var(--line-border)] bg-[var(--line-background)] px-4 pb-2 pt-0 md:px-8 md:pb-3"
    >
      <div className="mx-auto max-w-6xl">
        <StructureScanPreview onStageChange={setScanStage} />
        <div
          className={cn(
            "grid gap-5 border-t border-[var(--line-border)] px-5 py-4 transition-[opacity,transform] duration-700 md:grid-cols-[minmax(0,1fr)_auto] md:items-center md:gap-8 md:px-8",
            scanComplete
        ...
        >
          <div className="min-w-0">
            <p className="max-w-xl text-base font-medium leading-7 tracking-[-0.01em] text-[var(--line-foreground)]">
              {t("startup.scanResultDescription")}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 font-mono text-xs">
            <span className="inline-flex items-center gap-2 text-[var(--line-success)]">
              <CheckCircle2 aria-hidden="true" className="size-3.5" />
              {matchingRows} {t("startup.previewMatches")}
            </span>
            <span className="inline-flex items-center gap-2 text-[var(--line-accent)]">
              <AlertCircle aria-hidden="true" className="size-3.5" />
              {warningRows} {t("startup.previewWarnings")}
            </span>
            <span className="text-[var(--line-muted)]">
              {scanRows.length} {t("startup.previewEntries")}
            </span>
          </div>
        </div>
      </div>
...
const CapabilityCard = ({
  ...
  <article
    className={cn(
      "min-w-0 border-t border-[var(--line-border)] pt-5",
      index === 1 ? "md:pr-6" : "md:border-l md:pl-6 md:pr-6",
    )}
  >
    <div className="flex items-baseline gap-4">
      <span className="shrink-0 font-mono text-[10px] font-semibold tracking-[0.16em] text-[var(--line-accent)]">
        0{index}
      </span>
      <h3 className="min-w-0 text-left text-lg font-semibold tracking-[-0.02em] text-[var(--line-foreground)]">
        {title}
      </h3>
    </div>
    <p className="mt-3 min-h-0 max-w-sm text-pretty text-sm leading-6 text-[var(--line-muted)] md:min-h-[4.5rem]">
      {copy}
    </p>
...
const StartupFeatures = () => {
  ...
  return (
    <section
      id="features"
      className="w-full border-t border-[var(--line-border)] bg-[var(--line-background)] px-4 py-20 md:px-8 md:py-24"
    >
      <div className="mx-auto max-w-7xl">
        <div className="max-w-2xl">
          <h2 className="text-balance font-[var(--font-display)] text-3xl font-semibold leading-tight tracking-[-0.04em] text-[var(--line-foreground)] md:text-4xl">
            {t("startup.featuresTitle")}
          </h2>
          <p className="mt-5 max-w-xl text-pretty text-base leading-7 text-[var(--line-muted)]">
            {t("startup.featuresDescription")}
          </p>
        </div>
        <div className="mt-14 grid min-w-0 items-stretch gap-10 md:grid-cols-[repeat(3,minmax(0,1fr))] md:gap-0">
          <CapabilityCard
            index={1}
            title={t("startup.oneClickTitle")}
            copy={t("startup.oneClickCopy")}
          >
            <div className="font-mono text-xs text-[var(--line-foreground)]">
              <div className="flex items-center gap-3 border-y border-[var(--line-border)] py-3">
                <span className="text-[var(--line-muted)]">01</span>
                <code>anatomy.json</code>
              ...
              <div className="flex items-center gap-3 border-b border-[var(--line-border)] py-3">
                <span className="text-[var(--line-muted)]">02</span>
          ...
          <CapabilityCard
            index={2}
            title={t("startup.workflowTitle")}
            copy={t("startup.workflowCopy")}
          >
            <div className="border-y border-[var(--line-border)] py-4 font-mono text-[11px] leading-5 text-[var(--line-muted)]">
              <div className="mb-3 flex items-center gap-2 text-[var(--line-foreground)]">
                <FileCode2 className="size-4 text-[var(--line-accent)]" /> anatomy.json
              </div>
              <pre className="overflow-x-auto whitespace-pre">{`{\n  "root": {\n    "children": [...]\n  }\n}`}</pre>
            </div>
          </CapabilityCard>
          <CapabilityCard index={3} title={t("startup.edgeTitle")} copy={t("startup.edgeCopy")}>
            <div className="font-mono text-xs text-[var(--line-foreground)]">
              ...
              <div className="border-b border-[var(--line-border)] py-3 text-[var(--line-muted)]">
                local · pull request · CI
              </div>
            </div>
          </CapabilityCard>
        </div>
    ...
    >
      <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-8 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-4 max-w-xl text-pretty text-base leading-7 text-[var(--line-muted)]">
            {t("startup.ctaDescription")}
          </p>
        </div>
        <Button
          nativeButton={false}
          render={<Link to={route("/docs/installation")} />}
          className="h-11 rounded-md border border-[var(--line-foreground)] bg-[var(--line-foreground)] px-5 text-sm font-semibold text-[var(--line-background)] shadow-none hover:bg-[var(--line-muted)]"
        >
          {t("startup.ctaButton")}
          <ArrowRight size={15} />
        </Button>
    ...
    <footer className="overflow-hidden border-t border-[var(--line-border)] bg-[var(--line-background)] px-8 py-16 text-sm text-[var(--line-muted)]">
      <div className="mx-auto flex max-w-7xl flex-col items-start gap-12 md:flex-row md:px-8">
        <div className="w-full flex-1">
          <StartupLogo />
          <p className="mt-5 max-w-[260px] text-pretty leading-6">
            {t("startup.footerDescription")}
          </p>
          <p className="mt-8 text-xs">{t("startup.copyright")}</p>
        </div>
        <div className="grid w-full grid-cols-2 gap-10 md:w-[460px] md:grid-cols-3">
          <div>
            <p className="mb-4 font-semibold text-[var(--line-foreground)]">
              {t("startup.footerPages")}
            </p>
            <a className="block hover:text-[var(--line-foreground)]" href="#">
              {t("startup.home")}
            </a>
            <a className="mt-3 block hover:text-[var(--line-foreground)]" href="#scan">
              {t("startup.scanNav")}
            </a>
            <a className="mt-3 block hover:text-[var(--line-foreground)]" href="#features">
              {t("startup.capabilitiesNav")}
            </a>
            <a className="mt-3 block hover:text-[var(--line-foreground)]" href="#contact">
              {t("startup.contactNav")}
            </a>
          </div>
          <div>
            <p className="mb-4 font-semibold text-[var(--line-foreground)]">
              {t("startup.footerSocials")}
            </p>
            <a
              className="block hover:text-[var(--line-foreground)]"
            ...
            <a className="mt-3 block hover:text-[var(--line-foreground)]" href="#">
              X / Twitter
            </a>
            <a className="mt-3 block hover:text-[var(--line-foreground)]" href="#">
              Discord
          ...
          <div>
            ...
            <Link
              className="mt-3 block hover:text-[var(--line-foreground)]"
              to={route("/docs/installation")}
            >
              {t("startup.login")}
            </Link>
      ...
      <p
        aria-hidden="true"
        className="mt-16 whitespace-nowrap text-center text-5xl font-bold uppercase leading-none tracking-[0.02em] text-[var(--line-border-strong)] sm:text-8xl md:text-[13rem]"
      >
        ANATOMY CLI
      </p>
    </footer>
