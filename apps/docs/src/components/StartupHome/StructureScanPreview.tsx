...
const humanDefinitionRows = [
  ...
  { key: "definitionHomeRule", path: "src/routes/home.tsx" },
  { key: "definitionLibRule", path: "src/lib/" },
  { key: "definitionUtilsRule", path: "src/lib/utils.ts" },
  { key: "definitionHooksRule", path: "src/hooks/" },
  { key: "definitionAssetsRule", path: "src/assets/" },
  ...
  return (
    <div ref={previewRef} data-scan-stage={scanStage} className="grid overflow-hidden bg-[var(--line-surface)] text-left md:grid-cols-2">
      <div className="border-b border-[var(--line-border)] px-5 py-6 md:border-b-0 md:border-r md:px-8 md:py-8">
        ...
        <div className="relative mt-5 min-h-[20rem]">
          <div aria-hidden={showJson} className={`relative pr-3 transition-[opacity,transform] duration-300 ease-out ${showJson ? "pointer-events-none -translate-y-2 opacity-0" : "translate-y-0 opacity-100"}`}>
            ...
            <div className="mt-5 space-y-3 font-mono text-xs text-[var(--line-muted)]">
              {humanDefinitionRows.map((row, index) => (
                <div
                  ...
                  key={row.key}
                >
                  <span className="w-5 shrink-0 text-[10px] text-[var(--line-accent)]">{String(index + 1).padStart(2, "0")}</span>
