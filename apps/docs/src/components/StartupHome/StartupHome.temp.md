...
StartupHome 原文件同时承担页面编排、导航状态、首屏动画、扫描预览、能力展示、CTA 和页脚渲染。按视觉区块拆成独立组件后，根组件只负责组合，各区块保留自己的状态和副作用，不通过父组件层层传递业务数据。扫描数据继续放在扫描预览组件附近，避免为了拆分制造额外抽象；现有 RhombusPattern 作为独立背景组件保留。
